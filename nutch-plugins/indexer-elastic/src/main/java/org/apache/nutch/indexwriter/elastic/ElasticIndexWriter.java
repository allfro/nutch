...
public class ElasticIndexWriter implements IndexWriter {
  ...
  public void open(Configuration configuration, String name) throws IOException {
    clusterName = configuration.get(ElasticConstants.CLUSTER);

    host = configuration.get(ElasticConstants.HOST);
    port = configuration.getInt(ElasticConstants.PORT, 9300);

    Builder settingsBuilder = ImmutableSettings.settingsBuilder().classLoader(
        Settings.class.getClassLoader());

    BufferedReader reader = new BufferedReader(
        configuration.getConfResourceAsReader("elasticsearch.conf"));
    String line;
    String parts[];
    ...
    defaultIndex = configuration.get(ElasticConstants.INDEX, "nutch");
    maxBulkDocs = configuration.getInt(ElasticConstants.MAX_BULK_DOCS,
        DEFAULT_MAX_BULK_DOCS);
    maxBulkLength = configuration.getInt(ElasticConstants.MAX_BULK_LENGTH,
