...
public class SolrUtils {
  ...
  public static SolrServer getSolrServer(Configuration configuration)
      throws MalformedURLException {

    boolean auth = configuration.getBoolean(SolrConstants.USE_AUTH, false);

    CredentialsProvider credentialsProvider = new BasicCredentialsProvider();
    // Check for username/password
    if (auth) {
      String username = configuration.get(SolrConstants.USERNAME);
      LOG.info("Authenticating as: {}", username);
      AuthScope scope = new AuthScope(AuthScope.ANY_HOST, AuthScope.ANY_PORT,
          AuthScope.ANY_REALM, AuthScope.ANY_SCHEME);
      credentialsProvider.setCredentials(scope, 
          new UsernamePasswordCredentials(username, configuration.get(SolrConstants.PASSWORD)));
    }
    CloseableHttpClient client = 
        HttpClientBuilder.create().setDefaultCredentialsProvider(credentialsProvider).build();

    String solrServer = configuration.get(SolrConstants.SERVER_TYPE, "http");
    String zkHost = configuration.get(SolrConstants.ZOOKEEPER_URL, null);
    String solrServerUrl = configuration.get(SolrConstants.SERVER_URL);
    ...
    case "http":
      ...
      break;
    case "lb":
      String[] lbServerString = configuration.get(SolrConstants.LOADBALANCE_URLS).split(",");
