...
import org.apache.nutch.io.Metadata;
...
public class FileResponse {
    ...
    private static Logger LOG = LoggerFactory.getLogger(FileResponse.class);
    private String orig;
    private String base;
    ...
    private void getFileAsHttpResponse(java.io.File f) throws FileException,
        ...
        if (offset < len) { // keep whatever already have, but issue a warning
            if (LOG.isWarnEnabled()) {
                LOG.warn("not enough bytes read from file: {}", f.getPath());
            }
        }
        is.close();

        // set headers
        headers.set(Response.CONTENT_LENGTH, Long.toString(size));
