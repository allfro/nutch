...
public class ParseOutputFormat extends SequenceFileOutputFormat<Text, Parse> {
    ...
    @Override
    public RecordWriter<Text, Parse> getRecordWriter(TaskAttemptContext context) throws IOException, InterruptedException {
        Configuration configuration = context.getConfiguration();

        if (configuration.getBoolean("parse.filter.urls", true)) {
            filters = new URLFilters(configuration);
        }

        if (configuration.getBoolean("parse.normalize.urls", true)) {
            normalizers = new URLNormalizers(configuration, URLNormalizers.SCOPE_OUTLINK);
        }

        this.scfilters = new ScoringFilters(configuration);
        final int interval = configuration.getInt("db.fetch.interval.default", 2592000);
        final boolean ignoreExternalLinks = configuration.getBoolean(
                "db.ignore.external.links", false);
        int maxOutlinksPerPage = configuration.getInt("db.max.outlinks.per.page", 100);
        final boolean isParsing = configuration.getBoolean("fetcher.parse", true);
        ...
        final String[] parseMDtoCrawlDB = configuration.get("db.parsemeta.to.crawldb", "").split(" *, *");
        ...
        org.apache.hadoop.io.SequenceFile.Writer.Option tProgressOpt = SequenceFile.Writer.progressable(context);
        org.apache.hadoop.io.SequenceFile.Writer.Option tCompOpt = SequenceFile.Writer.compression(CompressionType.RECORD);

        final MapFile.Writer textOut = new MapFile.Writer(configuration, text,
        ...
        final MapFile.Writer dataOut = new MapFile.Writer(configuration, data,
                dKeyClassOpt, dValClassOpt, dCompOpt, dProgressOpt);

        FileSystem fs = FileSystem.get(configuration);
        final SequenceFile.Writer crawlOut = SequenceFile.createWriter(configuration, SequenceFile.Writer.file(crawl),
            ...
            public void write(Text key, Parse parse) throws IOException {
                ...
                for (int i = 0; i < links.length && validCount < outlinksToStore; i++) {
                    ...
                    try {
                        ...
                        LOG.warn("Cannot filter init score for url {}, using default: {}", key, e.getMessage());
